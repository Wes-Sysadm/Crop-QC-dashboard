...
- Admin -> Current Lots supports a production-safe EBS starting room inventory import from Compu-Tech. The built-in CSV stores Facility, SubLocation, Compu-Tech RoomCode, Variety, LotNumber, BinCount, and Source. Imports are previewed, map room codes case-insensitively (`evanca*` -> Evans, `blueca*` -> BM, `lambca*` -> Lamb), match Lot # against Grower Lots when possible, warn on unknown Lot #, and create audited `StartingInventoryImport` adjustments without deleting or overwriting receipts, samples, photos, or historical QC data.
