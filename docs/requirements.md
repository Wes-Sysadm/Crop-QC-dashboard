...
- Admin -> Current Lots supports a production-safe EBS starting room inventory import from Compu-Tech. The built-in CSV stores Facility, SubLocation, Crop QC Room Name, Compu-Tech Room Code, Variety, LotNumber, BinCount, and Source. Crop QC room names are primary in the UI and dashboard; Compu-Tech room codes are retained for import and future feed matching. Imports are previewed, map room codes case-insensitively (`evanca05` -> `Evans-5`, `evanca12` -> `Evans-12`, `Evanca01` -> `Evans-01`, `Blueca04` -> `BM-4`, `blueca01` -> `BM-1`, `Lambca17` -> `Lamb-17`), match Lot # against Grower Lots when possible, warn on unknown Lot #, and create audited `StartingInventoryImport` adjustments without deleting or overwriting receipts, samples, photos, or historical QC data.
- Dashboard room summaries must count bins and lots by exact current room inventory only. EBS starting inventory rows count toward current room bins/lots after import; Door and Lot samples remain observational and must not add bins.
