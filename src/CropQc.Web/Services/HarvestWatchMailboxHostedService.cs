    ...
    ILogger<HarvestWatchMailboxHostedService> logger) : BackgroundService
{
    ...
    private async Task PollAsync(CancellationToken cancellationToken)
    {
        ...
        var token = await credentialStore.GetAccessTokenAsync(wes, cancellationToken);
        ...
        using var listRequest = new HttpRequestMessage(HttpMethod.Get, $"https://gmail.googleapis.com/gmail/v1/users/me/messages?q={query}&maxResults=100");
        ...
        if (!listResponse.IsSuccessStatusCode) { logger.LogWarning("HarvestWatch Gmail list returned {StatusCode}.", listResponse.StatusCode); return; }
    ...
    private sealed class GmailMessageList { [JsonPropertyName("messages")] public List<GmailMessageReference>? Messages { get; set; } }
