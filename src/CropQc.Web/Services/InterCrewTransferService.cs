    ...
    public async Task<InterCrewWriteResult> DispatchAsync(InterCrewDispatchForm form, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var transfer = new InterCrewTransfer
            {
                ...
                CreatedAt = now,
                RequiresTruckReceipt = truckReceiptOptions?.Enabled == true && TruckReceiptRoutes.RequiresReceiptForGroup(option.Facility, form.DestinationCustodyGroup),
        ...
        catch (Exception exception) when (exception is InvalidOperationException or DbUpdateException || InventoryMovementConcurrency.IsConflict(exception))
        {
            ...
            return Fail(InventoryMovementConcurrency.IsConflict(exception) ? InventoryMovementConcurrency.RefreshMessage : exception.Message);
        }
    }
