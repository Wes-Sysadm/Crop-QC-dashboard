...
RUN dotnet restore src/CropQc.QcStation.WinForms/CropQc.QcStation.WinForms.csproj -r win-x86 -p:TargetFramework=net9.0-windows -p:EnableWindowsTargeting=true -p:Platform=x86 -p:PlatformTarget=x86 -p:RuntimeIdentifier=win-x86 -p:SelfContained=false
...
RUN echo "Publishing QC Station WinForms payload" \
    ...
    && dotnet publish src/CropQc.QcStation.WinForms/CropQc.QcStation.WinForms.csproj -c Release -r win-x86 --self-contained false -p:TargetFramework=net9.0-windows -p:EnableWindowsTargeting=true -p:Platform=x86 -p:PlatformTarget=x86 -p:RuntimeIdentifier=win-x86 -p:SelfContained=false -p:PublishSingleFile=false -o src/CropQc.Web/App_Data/QcStationWinForms --no-restore \
