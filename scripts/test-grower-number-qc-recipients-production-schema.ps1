param([int]$HostPort = 55473, [switch]$KeepContainer)

$ErrorActionPreference = 'Stop'
$repositoryRoot = Split-Path -Parent $PSScriptRoot
$containerName = "cropqc-grower-recipient-schema-$PID"
$password = 'cropqc-disposable-grower-recipients-only'
$previousMigration = '20260824233548_AddPackoutDocumentStorageMetadata'
$expectedMigration = '20260826063718_AddGrowerNumberQcRecipients'
$containerScriptRoot = '/tmp/cropqc-grower-recipients'
...
function New-Database([string]$Name) {
    if ($Name -notmatch '^cropqc_grower_recipients_') { throw "Refusing non-disposable database '$Name'." }
...
function Invoke-Gate([string]$Database) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "721-object gate failed for $Database." }
    }
...
WITH signatures AS (
  SELECT 'columns='||md5(string_agg(concat_ws('|',ordinal_position,column_name,data_type,coalesce(character_maximum_length::text,''),is_nullable,column_default),';' ORDER BY ordinal_position)) value
  FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='GrowerReportRecipients'
  UNION ALL
...
try {
    ...
    foreach ($name in @(
        'verify-packout-document-storage.sql',
        'preflight-grower-number-qc-recipients.sql',
        'apply-grower-number-qc-recipients-schema.sql',
        'verify-grower-number-qc-recipients.sql')) {
        Invoke-Docker -Arguments @('cp',(Join-Path $repositoryRoot "scripts\postgresql\$name"),"${containerName}:${containerScriptRoot}/$name")
    }

    $fresh = 'cropqc_grower_recipients_fresh'
    New-Database $fresh
    Invoke-EfUpdate $fresh $null
    Invoke-Script $fresh 'verify-grower-number-qc-recipients.sql'
    Invoke-Gate $fresh

    $upgrade = 'cropqc_grower_recipients_upgrade'
    ...
    Invoke-Script $upgrade 'preflight-grower-number-qc-recipients.sql'
    Invoke-Script $upgrade 'apply-grower-number-qc-recipients-schema.sql'
    Invoke-Script $upgrade 'apply-grower-number-qc-recipients-schema.sql'
    Invoke-Gate $upgrade
    ...
    $partial = 'cropqc_grower_recipients_partial'
    New-Database $partial
    Invoke-EfUpdate $partial $previousMigration
    Invoke-Sql $partial 'create table "GrowerReportRecipients" ("Id" integer generated by default as identity primary key);'
    Assert-ScriptFails $partial 'preflight-grower-number-qc-recipients.sql'

    $wrong = 'cropqc_grower_recipients_wrong'
    Invoke-Docker -Arguments @('exec',$containerName,'createdb','-U','postgres','-T',$upgrade,$wrong)
    Invoke-Sql $wrong 'alter table "GrowerReportRecipients" alter column "EmailAddress" type character varying(319);'
    Assert-ScriptFails $wrong 'preflight-grower-number-qc-recipients.sql'

    $rollback = 'cropqc_grower_recipients_rollback'
    New-Database $rollback
    Invoke-EfUpdate $rollback $previousMigration
    $prior = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        & docker exec $containerName psql -X -v ON_ERROR_STOP=1 -U postgres -d $rollback -c "set cropqc.test_force_grower_recipient_failure='on'" -f "$containerScriptRoot/apply-grower-number-qc-recipients-schema.sql" *> $null
        $forcedExit = $LASTEXITCODE
    }
    finally { $ErrorActionPreference = $prior }
    if ($forcedExit -eq 0) { throw 'Forced apply failure unexpectedly passed.' }
    if ((Invoke-Scalar $rollback "select count(*) from information_schema.tables where table_schema=current_schema() and table_name='GrowerReportRecipients';") -ne '0') {
        throw 'Forced apply failure left the GrowerReportRecipients table behind.'
    }

    Write-Output 'Fresh PostgreSQL 18 EF migration, exact verifier, and 721-object gate: PASS'
