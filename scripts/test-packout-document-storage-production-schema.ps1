param([int]$HostPort = 55467, [switch]$KeepContainer)

$ErrorActionPreference = 'Stop'
$repositoryRoot = Split-Path -Parent $PSScriptRoot
$containerName = "cropqc-packout-doc-schema-$PID"
$password = 'cropqc-disposable-packout-documents-only'
$previousMigration = '20260823040226_AddActualRunSalesDeskAttribution'
...
function New-Database([string]$Name) {
    if ($Name -notmatch '^cropqc_packout_docs_') { throw "Refusing non-disposable database '$Name'." }
...
function Invoke-Gate([string]$Database) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "698-object gate failed for $Database." }
    }
...
WITH signatures AS (
  ...
  FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='PackoutReportSources'
  UNION ALL
  ...
  WHERE n.nspname=current_schema() AND t.relname='PackoutReportSources'
  UNION ALL
...
try {
    ...
    $fresh = 'cropqc_packout_docs_fresh'
    New-Database $fresh
    Invoke-EfUpdate $fresh $null
    Invoke-Script $fresh 'verify-packout-document-storage.sql'
    Invoke-Gate $fresh

    $upgrade = 'cropqc_packout_docs_upgrade'
    ...
    Invoke-Script $upgrade 'preflight-packout-document-storage.sql'
    Invoke-Script $upgrade 'apply-packout-document-storage-schema.sql'
    Invoke-Script $upgrade 'apply-packout-document-storage-schema.sql'
    Invoke-Gate $upgrade
    ...
    $partial = 'cropqc_packout_docs_partial'
    New-Database $partial
    Invoke-EfUpdate $partial $previousMigration
    Invoke-Sql $partial 'alter table "PackoutReportSources" add "StorageKey" character varying(500);'
    Assert-ScriptFails $partial 'preflight-packout-document-storage.sql'

    $wrong = 'cropqc_packout_docs_wrong'
    Invoke-Docker -Arguments @('exec',$containerName,'createdb','-U','postgres','-T',$upgrade,$wrong)
    Invoke-Sql $wrong 'alter table "PackoutReportSources" alter column "StoragePath" type character varying(999);'
    Assert-ScriptFails $wrong 'preflight-packout-document-storage.sql'

    $rollback = 'cropqc_packout_docs_rollback'
    New-Database $rollback
    Invoke-EfUpdate $rollback $previousMigration
    $prior = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        & docker exec $containerName psql -X -v ON_ERROR_STOP=1 -U postgres -d $rollback -c "set cropqc.test_force_packout_document_failure='on'" -f "$containerScriptRoot/apply-packout-document-storage-schema.sql" *> $null
        $forcedExit = $LASTEXITCODE
    }
    finally { $ErrorActionPreference = $prior }
    if ($forcedExit -eq 0) { throw 'Forced apply failure unexpectedly passed.' }
    if ((Invoke-Scalar $rollback "select count(*) from information_schema.columns where table_schema=current_schema() and table_name='PackoutReportSources' and column_name='StorageKey';") -ne '0') {
        throw 'Forced apply failure left partial Packout document columns.'
    }

    Write-Output 'Fresh PostgreSQL 18 EF migration and 698-object gate: PASS'
