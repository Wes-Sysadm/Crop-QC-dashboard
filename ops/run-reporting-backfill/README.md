...
This package is bound to production backup run 41, created from deployed commit `2b840ef23b505300a7c0be0ca2b011f5355a0e98` on August 5, 2026. The independently retrieved ZIP is `cropqc-production-predeployment-20260805-024804.zip`, 1,235,296 bytes, SHA-256 `2fc81bc3255870a439c298acabad8fcb2b28d5e99e90d1a0e59f15a04786a1ae`.
...
No Actual Runs or Bins Run lines were added between backup runs 40 and 41. Current QC, receipt, room-inventory-adjustment, email-log, and projection state did advance, so every protected count and fingerprint was regenerated from run 41.
...
`preflight.sql` requires the exact run-41 semantic identities, schema, migration-history drift state, line classifications, operational metadata, and protected table fingerprints. It accepts only the exact initial state or the exact already-applied state. `apply.sql` accepts only null or already-exact attribution values, writes one audit per changed user/run/line, and is idempotent. It never changes quantities, inventory, receipts, transfers, rooms, Grower Lots, revisions, statuses, QC, projections, expectations, or packouts.

The disposable run-41 rehearsal changed `2 users / 7 Actual Runs / 11 reporting lines` on the first apply and `0 / 0 / 0` on the second. Both verifications reproduced WP 1,100 and EBS 388, excluded the 173-bin grower-number issue, preserved all 27 pre-2026 lines, and reproduced every protected operational fingerprint. The clean rehearsal then applied and verified `20260805014812_AddReceiptInventoryOverrides`; the candidate application gate checked 195 objects and found zero blocking deduction-invariant issues. `test-semantic-guard.ps1` passed 15 disposable PostgreSQL scenarios proving login/account changes pass while target identity, employment, reviewed run, new authoritative-line, and protected-relationship changes fail.
