...
public sealed class FtaDllPressureReaderTests
{
    ...
    [Fact]
    public void Incorrect_format_guidance_calls_out_x86_fta_dll_with_x64_borland_dependency()
    {
        var configuredDll = new PeFileInspection(@"C:\Program Files\FTADLL\FTA_DLL.dll", true, PeArchitecture.X86, 100, DateTimeOffset.UtcNow, "1.0", null);
        var configuredBorland = new PeFileInspection(@"C:\Program Files\FTADLL\borlndmm.dll", true, PeArchitecture.X64, 50, DateTimeOffset.UtcNow, "1.0", null);
        var x86Borland = new PeFileInspection(@"C:\Windows\SysWOW64\borlndmm.dll", true, PeArchitecture.X86, 50, DateTimeOffset.UtcNow, "1.0", null);

        var guidance = FtaConnectionDiagnostics.BuildIncorrectFormatGuidance(
            System.Runtime.InteropServices.Architecture.X86,
            configuredDll,
            configuredBorland,
            hasAvailableConfiguredCom: true,
            [configuredBorland, x86Borland]);

        Assert.Contains("FTA_DLL.dll is 32-bit, but borlndmm.dll is 64-bit", guidance);
        Assert.Contains("Install/copy the vendor 32-bit borlndmm.dll next to FTA_DLL.dll", guidance);
        Assert.Contains(@"C:\Windows\SysWOW64\borlndmm.dll", guidance);
        Assert.Contains(@"C:\Program Files\FTADLL\FTA_DLL.dll", guidance);
        Assert.DoesNotContain("run the QC Station as x86", guidance, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void Diagnostic_report_includes_dll_copy_comparison_and_com_blocker_conclusion()
    {
        ...
        Assert.Contains(@"C:\Program Files\FTADLL\borlndmm.dll", report.ReportText);
        Assert.Contains("Dependency guidance:", report.ReportText);
    ...
    [Fact]
    public async Task Win32_error_193_load_error_reports_x86_dependency_guidance_when_process_is_x86()
    {
        var tempFolder = CreateTempDllFolder(FtaDllPressureReader.DefaultFtaDllFileName);
        var configuration = new StationConfiguration
        {
        ...
        var reader = new FtaDllPressureReader(configuration, new FakeNativeDllLoader(DllLoadResult.ArchitectureMismatch("%1 is not a valid Win32 application. (Win32 error 193)", "Win32 LoadLibraryEx", 193)));

        var status = await reader.InitializeAsync();

        Assert.False(status.IsInitialized);
        Assert.Contains("Win32 error 193", status.ErrorMessage);
        Assert.Contains("DLL load exception type: Win32 LoadLibraryEx", status.StatusMessage);
        Assert.Contains("DLL load HResult: 0x000000C1", status.StatusMessage);
        if (System.Runtime.InteropServices.RuntimeInformation.ProcessArchitecture == System.Runtime.InteropServices.Architecture.X86)
        {
            Assert.Contains("QC Station is already running x86", status.ErrorMessage);
            Assert.DoesNotContain("run the QC Station as x86", status.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        }
    }

    [Fact]
    public async Task Open_setup_reports_dll_load_blocker_when_probe_failed()
    {
        var tempFolder = CreateTempDllFolder(FtaDllPressureReader.DefaultFtaDllFileName);
        var configuration = new StationConfiguration
        {
        ...
        await reader.InitializeAsync();
        var status = await reader.OpenSetupAsync();

        Assert.False(status.IsInitialized);
        Assert.Contains("Cannot open FTA Setup because FTA_DLL.dll failed to load", status.ErrorMessage);
        Assert.Contains("Win32 error 193", status.ErrorMessage);
    }
